...
class Map{
    public:
        Map(cv::Mat& image, State& origin, long double& resolution){
            this->image = image;
            this->origin = origin;
    ...
    private:
        ...
        long double epsilon; /// Epsilon is a parameter that specifies the maximum distance between the original curve and its approximation, where smaller epsilon values result in more precise approximations.
};
#endif